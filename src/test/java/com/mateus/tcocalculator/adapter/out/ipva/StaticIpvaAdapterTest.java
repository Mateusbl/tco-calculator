...
public class StaticIpvaAdapterTest {
    ...
    void calculateGasIpva(){
        Vehicle vehicle = new Vehicle("Toyota","etios",2023, BigDecimal.valueOf(12.5),"001004-9", FuelType.GASOLINE );
        StaticIpvaAdapter adapter = new StaticIpvaAdapter();
        BigDecimal rate = adapter.findRate(vehicle, "SP");
    ...
    void calculateEletricIpva(){
            Vehicle vehicle = new Vehicle("Tesla", "Model 3", 2023, BigDecimal.valueOf(0), "999999-9", FuelType.ELECTRIC);
            StaticIpvaAdapter adapter = new StaticIpvaAdapter();
            BigDecimal rate = adapter.findRate(vehicle, "SP");
    ...
    void calculateHybridIpva(){
            Vehicle vehicle = new Vehicle("Toyota", "Corolla", 2023, BigDecimal.valueOf(0), "999999-9", FuelType.HYBRID);
            StaticIpvaAdapter adapter = new StaticIpvaAdapter();
            BigDecimal rate = adapter.findRate(vehicle, "SP");
...
void shouldThrowWhenStateNotSupported(){
    ...
    assertThatThrownBy(() -> adapter.findRate(vehicle, "XX"))
